// Sutable_data_struct.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
void delcom(FILE* input, FILE* output) {
    ...
    while ((simb = fgetc(input)) != EOF) {
        ...
        case 2: // Потенциальный комментарий
            if (simb == '/') {
                flag = 3; // Однострочный комментарий
            }
            else if (simb == '*') {
                flag = 4; // Многострочный комментарий
            }
            else {
                fputc('/', output);
                fputc(simb, output);
        ...
        case 3: // Однострочный комментарий
            if (simb == '\\' && fgetc(input) == '\n') {
                continue;
            }
            if (simb == '\n') {
        ...
        case 4: // Многострочный комментарий
            if (presimb == '*' && simb == '/') {
                flag = 0;
            }
            presimb = simb;
    ...
    // Завершающий слеш, если был потенциальный комментарий
    if (flag == 2) {
        fputc('/', output);
    }
}
